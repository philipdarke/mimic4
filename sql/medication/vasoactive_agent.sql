...
DROP TABLE IF EXISTS derived.vasoactive_agent; CREATE TABLE derived.vasoactive_agent AS
WITH tm AS (
  SELECT
    stay_id,
    starttime AS vasotime
  FROM derived.dobutamine
  UNION
  SELECT
    stay_id,
    starttime AS vasotime
  FROM derived.dopamine
  UNION
  SELECT
    stay_id,
    starttime AS vasotime
  FROM derived.epinephrine
  UNION
  SELECT
    stay_id,
    starttime AS vasotime
  FROM derived.norepinephrine
  UNION
  SELECT
    stay_id,
    starttime AS vasotime
  FROM derived.phenylephrine
  UNION
  SELECT
    stay_id,
    starttime AS vasotime
  FROM derived.vasopressin
  UNION
  SELECT
    stay_id,
    starttime AS vasotime
  FROM derived.milrinone
  UNION
  SELECT
    stay_id,
    endtime AS vasotime
